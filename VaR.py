...
tickers = ["AAPL","BP","IBM","GE","KO"]
stocks = yf.download(tickers, start = "2010-01-01", end = "2020-01-01")
close_values = stocks.loc[:, "Close"]

close_values.to_csv("/Users/guillaume/Downloads/Perso/Informatique/OCaml/data.csv")
...
def getData(stocks, start, end):
    ...
    returns = pct_change.describe().T.loc[:,["mean", "std"]]
...
def historicalCVaR(returns, alpha=5):
    ...
    if isinstance(returns, pd.Series):
        belowVaR = returns
        lt= historicalVaR(returns, alpha=alpha)
